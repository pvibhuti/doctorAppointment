...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    fetchData('/getDoctorData', (data) => { dispatch(ActionCreators.GET_DOCTOR(data.existingDoctor)); })
    fetchData('/upcomingAppointment', (data) => { setAppointments(data.upcomingAppointments) })
    fetchData('/upcomingAppointment', (data) => { setAppointments(data.upcomingAppointments) })
  ...
  const handleAppointmentAction = async (action, appointmentId) => {
    ...
    post(url)
      .then((response) => {
        ...
        const msg = `Appointment ${action === 1 ? 'Approved' : 'Rejected'} Successfully.`;
